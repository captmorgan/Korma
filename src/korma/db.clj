...
(defn- query-params [sql-str params]
  (let [sql-vec (if (vector? sql-str)
                  sql-str
                  [sql-str])]
    (vec (concat sql-vec params))))
...
(defn- exec-sql [{:keys [results sql-str params options]}]
  (let [{:keys [keys fields]} (:naming (or options @conf/options))]
    (try
      (case results
        :results (jdbc/query *current-conn*
                             (query-params sql-str params)
                             :row-fn (->row-fn fields)
                             :identifiers keys)
